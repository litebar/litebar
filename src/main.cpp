...
uint256 hashGenesisBlock("0x761cd0df12d534691e263113074a22a01f48e6c929047acdaf4f577d53d17b7c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Litebar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Litebar Signed Message:\n";
...
{
    // Litebar: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Litebar: 420k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 5 * 60; // Litebar: 5 minute difficulty retarget
static const int64 nTargetSpacing = 5 * 60; // Litebar: 5 minute block time
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Litebar: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Litebar: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Litebar: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x65844f27d7944fa69827e4d68e99fe3d3b5a07037f6268d2eac6128fbd097208");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "LA Times 15/Jan/2014 Big, old trees keep growing and capturing carbon, study finds";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390271290; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4953193;
        ...
        assert(block.hashMerkleRoot == uint256("0x02c15dc2a5083a94fdc1ef510cfaa2a33d9a3fb4ad93809a4556f4b78a0201c1"));	
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Litebar: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LitebarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LitebarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LitebarMiner(CWallet *pwallet)
{
    printf("LitebarMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("litebar-miner");
    ...
    try { loop {
        ...
        printf("Running LitebarMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LitebarMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LitebarMiner, pwallet));
}
